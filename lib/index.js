...
function hasOption (defValue, options, property, lookingFor) {
  ...
  return function validationMiddleware (req, res, next) {
    ...
    for (let i = 0; i < locations.length; i++) {
      ...
      if (schema[location]) {
        ...
        const allowUnknown = hasOption(true, options, 'allowUnknown', location)
        const stripUnknown = hasOption(false, options, 'stripUnknown', location)
